...
BEGIN_MESSAGE_MAP(CmainDlg, CBaseDialog)
	...
	ON_COMMAND(ID_4PHONE_LOGIN, OnFourPhoneLogin)
...
BOOL CmainDlg::OnInitDialog()
{
	...
	int pageWidth = pageRect.Width() < clientRect.Width()
		? pageRect.Width()
		: clientRect.Width();
	int offsetX = (clientRect.Width() - pageWidth) / 2;
	pageDialer->SetWindowPos(
		NULL,
		offsetX,
		offset,
		pageWidth,
		pageRect.Height() + heightAdd,
		SWP_NOZORDER);
	AutoMove(pageDialer->m_hWnd, 50, 0, 0, 100);
...
void CmainDlg::OnFourPhoneLogin()
{
	FourPhoneSipConfig sipConfig;
	CString extensionId;
	if (!CFourPhoneProvisioning::ShowLogin(
		this,
		sipConfig,
		extensionId)) {
		return;
	...
	const bool hadAccount = accountSettings.accountId != 0;
	CString error;
	if (!CFourPhoneProvisioning::SaveAccount(
		CFourPhoneProvisioning::BuildAccount(sipConfig),
		error)) {
...
void CmainDlg::OnFourPhoneLogout()
{
	...
	CString error;
	if (!CFourPhoneProvisioning::Logout(error) && !error.IsEmpty()) {
...
void CmainDlg::MainPopupMenu(bool isMenuButton)
{
	CString str;
	CPoint point;
	UINT menuFlags = TPM_TOPALIGN | TPM_RIGHTBUTTON;
	if (isMenuButton) {
		CWnd* menuButton = mainDlg->GetDlgItem(IDC_MAIN_MENU);
		CRect rect;
		menuButton->GetWindowRect(rect);
		if (langPack.rtl) {
			point = CPoint(rect.right, rect.bottom);
	...
	tracker->AppendMenu(MF_SEPARATOR);
	tracker->AppendMenu(
		MF_STRING | (accountSettings.alwaysOnTop ? MF_CHECKED : 0),
		ID_ALWAYS_ON_TOP,
		Translate(_T("Always on Top")));
	tracker->AppendMenu(
		MF_STRING | (!accountSettings.enableLog
...
void CmainDlg::PJCreateRaw()
{
	...
	*/
	...
	cfg.tls_setting.require_client_cert = PJ_FALSE;
...
LRESULT CmainDlg::OnDpiChanged(WPARAM wParam, LPARAM lParam)
{
	const LRESULT result =
		CBaseDialog::OnDpiChanged(wParam, lParam);
	FourPhoneTheme::CreateFont(this, m_HeaderLogoFont, 11, FW_BOLD);
	FourPhoneTheme::CreateFont(this, m_HeaderStatusFont, 8, FW_SEMIBOLD);
	if (::IsWindow(m_bar.GetSafeHwnd())) {
		m_bar.GetStatusBarCtrl().SetFont(&m_HeaderStatusFont);
	}
	Invalidate(TRUE);
	return result;
}
