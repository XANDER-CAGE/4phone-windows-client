...
namespace
{
    const CString kDpapiPrefix = _T("dpapi1:");

    bool LegacyDecrypt(CString& str)
    {
        bool res = false;
    ...
    bool DpapiDecrypt(CString& str)
    {
        if (str.Left(kDpapiPrefix.GetLength()) != kDpapiPrefix) {
            return false;
        }

        CByteArray encrypted;
        MSIP::String2Bin(str.Mid(kDpapiPrefix.GetLength()), &encrypted);
        if (encrypted.GetSize() == 0) {
            return false;
        }

        DATA_BLOB input;
        input.pbData = encrypted.GetData();
        ...
        bool success =
            output.pbData != NULL &&
            output.cbData % sizeof(TCHAR) == 0;
        if (success) {
            str.SetString(
                reinterpret_cast<LPCTSTR>(output.pbData),
                static_cast<int>(output.cbData / sizeof(TCHAR)));
        }
        if (output.pbData) {
    ...
    CString DpapiEncrypt(const CString& str)
    {
        if (str.IsEmpty()) {
            return _T("");
        }
        ...
        input.pbData = reinterpret_cast<BYTE*>(
            const_cast<LPTSTR>(str.GetString()));
        ...
        CByteArray encrypted;
        encrypted.SetSize(output.cbData);
        CopyMemory(
            encrypted.GetData(),
            output.pbData,
            output.cbData);
...
bool IniDecrypt(CString& str)
{
    ...
    if (str.Left(kDpapiPrefix.GetLength()) == kDpapiPrefix) {
        return DpapiDecrypt(str);
    }
    return LegacyDecrypt(str);
}

CString IniEncrypt(CString str)
{
    return DpapiEncrypt(str);
}
...
bool AccountSettings::AccountLoad(int id, Account* account)
{
    ...
    if (!account->password.IsEmpty()) {
        const bool isDpapi =
            account->password.Left(7) == _T("dpapi1:");
        if (!IniDecrypt(account->password)) {
            ...
            const CString encrypted =
                IniEncrypt(account->password);
...
bool AccountSettings::AccountSave(int id, Account* account)
{
    ...
    if (!account->rememberPassword) {
        const BOOL usernameWritten = WritePrivateProfileString(
            section,
            _T("username"),
            _T(""),
            iniFile);
        const BOOL passwordWritten = WritePrivateProfileString(
            section,
            _T("password"),
            _T(""),
            iniFile);
        passwordSaved =
            usernameWritten != FALSE && passwordWritten != FALSE;
    }
    else {
        ...
        const CString encryptedPassword = IniEncrypt(account->password);
        if (!account->password.IsEmpty() && encryptedPassword.IsEmpty()) {
            passwordSaved = false;
            WritePrivateProfileString(
                section,
                _T("password"),
