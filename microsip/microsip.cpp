...
BOOL CmicrosipApp::InitInstance()
{
	...
	if (cmdReset || cmdResetNoAsk) {
		...
		if (cmdResetNoAsk || AfxMessageBox(Translate(_T("Do you want to delete user data and program settings? This action cannot be undone.")), MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2 | MB_SYSTEMMODAL) == IDYES) {
			if (cmdResetNoAsk || AfxMessageBox(Translate(_T("Are you sure you want to delete?")), MB_YESNO | MB_ICONQUESTION | MB_SYSTEMMODAL) == IDYES) {
				...
				CreateDirectory(accountSettings.appDataRoaming, NULL);
				CString logoutError;
				if (!CFourPhoneProvisioning::Logout(logoutError) && !cmdResetNoAsk) {
					AfxMessageBox(logoutError, MB_OK | MB_ICONWARNING | MB_SYSTEMMODAL);
				}
			}
		}
		return FALSE;
	...
	CString extensionId;
	CString provisioningError;
	const FourPhoneRestoreResult restoreResult =
		CFourPhoneProvisioning::TryRestoreSession(
		sipConfig,
		extensionId,
		provisioningError);
	if (restoreResult == FourPhoneRestoreSuccess) {
	...
	else if (restoreResult == FourPhoneRestoreLoginRequired ||
		!accountSettings.accountId ||
		accountSettings.account.username.IsEmpty() ||
		accountSettings.account.password.IsEmpty()) {
